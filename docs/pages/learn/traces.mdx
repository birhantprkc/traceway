...
You don't need to set any vendor attribute for this to work — OTLP's native trace_id is enough. (The Go SDK's manual `distributedTraceId` field continues to work for legacy `/api/report` flows and overrides the auto-derived value when set.)
...
1. A span is created in your app (by a framework's auto-instrumentation, the Traceway SDK, or manual OTel calls).
2. Child spans, attributes, and events accumulate against it.
3. The trace is exported via OTLP (or `/api/report` for the legacy Go SDK).
